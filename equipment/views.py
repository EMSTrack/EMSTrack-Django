...
from extra_views import ModelFormSet, UpdateWithInlinesView, CreateWithInlinesView
...
class EquipmentItemInline(ModelFormSet):
    model = EquipmentItem
    form_class = EquipmentItemForm
    factory_kwargs = {'extra': 0, 'can_delete': False}


class EquipmentSetCreateInline(ModelFormSet):
...
class EquipmentSetUpdateInline(ModelFormSet):
    model = EquipmentSetItem
    form_class = EquipmentSetItemForm
    factory_kwargs = {'extra': 0, 'can_delete': True}


class EquipmentHolderInline(ModelFormSet):
